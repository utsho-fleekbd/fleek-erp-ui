...
import { Search, Plus, ChevronLeft, ChevronRight, Download, Eye, Pencil, Trash2, ShoppingCart, Clock, Truck, XCircle, CheckCircle, RotateCcw } from "lucide-react";
...
const missingProducts = Array.from({ length: 12 }, (_, i) => ({
  id: i + 1,
  name: ["Laptop Stand", "USB Hub", "Wireless Mouse", "Keyboard", "Webcam", "Monitor Arm"][i % 6],
...
export default function OrdersPage() {
  ...
  const [viewMissing, setViewMissing] = useState<typeof missingProducts[0] | null>(null);
  const [deleteMissing, setDeleteMissing] = useState<typeof missingProducts[0] | null>(null);
    ...
    return (
      <AppLayout>
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between animate-fade-in">
          ...
          <Button className="gap-2"><Plus className="h-4 w-4" /> Report Missing</Button>
        </div>
        <div className="mb-4 relative w-full sm:w-64 animate-fade-in stagger-1">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
          <Input placeholder="Search..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }} className="pl-9 bg-secondary border-border" />
        </div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 animate-fade-in stagger-2">
          {paginatedMissing.map((p) => (
            <Card key={p.id} className="border-border bg-card hover:shadow-lg transition-all">
              <CardContent className="p-4">
                ...
                <div className="flex gap-1 mt-3">
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setViewMissing(p)}><Eye className="h-3.5 w-3.5" /></Button>
                  <Button variant="ghost" size="icon" className="h-7 w-7"><Pencil className="h-3.5 w-3.5" /></Button>
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => setDeleteMissing(p)}><Trash2 className="h-3.5 w-3.5" /></Button>
                </div>
              </CardContent>
            </Card>
          ))}
        ...
        <Dialog open={!!viewMissing} onOpenChange={() => setViewMissing(null)}>
          <DialogContent><DialogHeader><DialogTitle>{viewMissing?.name}</DialogTitle></DialogHeader>
            <div className="space-y-2"><p className="text-sm">Price: ${viewMissing?.price}</p><p className="text-sm">Missing Date: {viewMissing?.missingDate}</p>{viewMissing?.note && <p className="text-sm">Note: {viewMissing.note}</p>}</div>
          </DialogContent>
        </Dialog>
        <Dialog open={!!deleteMissing} onOpenChange={() => setDeleteMissing(null)}>
          <DialogContent><DialogHeader><DialogTitle>Delete Record</DialogTitle></DialogHeader>
            <p className="text-sm text-muted-foreground">Are you sure you want to delete <strong>{deleteMissing?.name}</strong>?</p>
            <div className="flex gap-2 justify-end"><Button variant="outline" onClick={() => setDeleteMissing(null)}>Cancel</Button><Button variant="destructive" onClick={() => setDeleteMissing(null)}>Delete</Button></div>
          </DialogContent>
        </Dialog>
      </AppLayout>
    );
  ...
  const orderStats = [
    { label: "All Orders", value: allOrders.length, icon: ShoppingCart, color: "text-primary" },
    { label: "Orders Today", value: 24, icon: ShoppingCart, color: "text-primary" },
    { label: "Sales Today", value: "$3,420", icon: ShoppingCart, color: "text-success" },
    { label: "All Pending", value: allOrders.filter(o => o.status === "pending").length, icon: Clock, color: "text-warning" },
    { label: "Delivered", value: allOrders.filter(o => o.status === "delivered").length, icon: CheckCircle, color: "text-success" },
    { label: "Returned", value: allOrders.filter(o => o.status === "returned").length, icon: RotateCcw, color: "text-destructive" },
  ];
  ...
  return (
    <AppLayout>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between animate-fade-in">
        <div><h1 className="text-2xl font-semibold">Orders</h1><p className="text-sm text-muted-foreground">{filtered.length} orders found</p></div>
        ...
        {orderStats.map((s) => (
          <Card key={s.label} className="border-border bg-card">
            <CardContent className="flex items-center gap-3 p-3">
              ...
              <div><p className="font-mono text-lg font-semibold">{s.value}</p><p className="text-[10px] text-muted-foreground">{s.label}</p></div>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <Card className="border-border bg-card animate-fade-in stagger-3">
        <CardContent className="p-0 overflow-x-auto">
          <table className="w-full min-w-[800px]">
            ...
            <tbody>
              {paginated.map((o) => (
                <tr key={o.id} className="border-b border-border last:border-0 hover:bg-secondary/50 cursor-pointer transition-colors" onClick={() => setSelectedOrder(o)}>
                  <td className="px-4 py-3 font-mono text-sm font-medium text-primary">{o.id}</td>
                  ...
                  <td className="px-3 py-3 text-sm text-muted-foreground">{o.paymentMethod}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Sheet open={!!selectedOrder} onOpenChange={() => setSelectedOrder(null)}>
        <SheetContent className="w-full sm:w-[480px] border-border bg-card sm:max-w-[480px]">
          {selectedOrder && (
            <>
              <SheetHeader><SheetTitle className="font-mono">{selectedOrder.id}</SheetTitle></SheetHeader>
              <div className="mt-6 space-y-4">
                {[
                  { label: "Status", val: <StatusBadge status={selectedOrder.status as OrderStatus} /> },
