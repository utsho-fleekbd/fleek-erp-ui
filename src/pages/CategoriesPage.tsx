...
const categories = Array.from({ length: 25 }, (_, i) => {
  const names = ["Electronics", "Clothing", "Sports", "Home & Garden", "Beauty", "Toys", "Books", "Food", "Automotive", "Jewelry"];
  const parents = ["", "Electronics", "", "Home & Garden", "", "", "", "Food", "", ""];
  const childrenData = [["Phones", "Laptops"], ["Men", "Women"], ["Running", "Gym"], ["Furniture"], ["Skincare"], [], ["Fiction"], ["Snacks"], ["Parts"], ["Rings"]];
  const businesses = [["Acme Store"], ["Acme Store", "Global Mart"], ["Acme Store"], ["Global Mart"], ["Acme Store", "Global Mart"]];
  return {
    id: i + 1,
...
export default function CategoriesPage() {
  ...
  const [deleteItem, setDeleteItem] = useState<typeof categories[0] | null>(null);
    ...
    <AppLayout>
        ...
        <Dialog open={addOpen} onOpenChange={setAddOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div className="space-y-2"><Label>Name</Label><Input placeholder="Category name" className="bg-secondary border-border" /></div>
              <div className="space-y-2"><Label>Parent Category</Label>
                <Select><SelectTrigger className="bg-secondary border-border"><SelectValue placeholder="None" /></SelectTrigger>
                  <SelectContent>{["None", "Electronics", "Clothing", "Sports"].map(c => <SelectItem key={c} value={c.toLowerCase()}>{c}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <Button className="w-full" onClick={() => setAddOpen(false)}>Create Category</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 animate-fade-in stagger-1">
        <div className="relative w-full sm:w-64">
          ...
          <Input placeholder="Search categories..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }} className="pl-9 bg-secondary border-border" />
        </div>
        <Select value={sortBiz} onValueChange={(v) => { setSortBiz(v); setPage(0); }}>
          <SelectTrigger className="w-40 bg-secondary border-border"><SelectValue placeholder="All Businesses" /></SelectTrigger>
          <SelectContent>
            ...
            <tbody>
              {paginated.map((c) => (
                <tr key={c.id} className="border-b border-border last:border-0 hover:bg-secondary/50 transition-colors">
                  ...
                  <td className="px-3 py-3"><div className="h-8 w-8 rounded bg-secondary" /></td>
                  <td className="px-3 py-3 text-sm text-muted-foreground">{c.parent}</td>
                  <td className="px-3 py-3">
                    <div className="flex flex-wrap gap-1">{c.children.map(ch => <Badge key={ch} variant="secondary" className="text-[10px]">{ch}</Badge>)}</div>
                  </td>
                  <td className="px-3 py-3">
                    <div className="flex flex-wrap gap-1">{c.businesses.map(b => <Badge key={b} variant="outline" className="text-[10px]">{b}</Badge>)}</div>
                  </td>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Parent: {viewItem.parent}</p>
                </div>
              </div>
              <div><Label className="text-muted-foreground text-xs">Children</Label>
                <div className="mt-1 flex flex-wrap gap-1">{viewItem.children.length ? viewItem.children.map(ch => <Badge key={ch} variant="secondary">{ch}</Badge>) : <span className="text-sm text-muted-foreground">None</span>}</div>
              </div>
              <div><Label className="text-muted-foreground text-xs">Businesses</Label>
                <div className="mt-1 flex flex-wrap gap-1">{viewItem.businesses.map(b => <Badge key={b} variant="outline">{b}</Badge>)}</div>
              </div>
            </div>
          )}
        </DialogContent>
        ...
        <DialogContent>
          ...
          {editItem && (
            <div className="space-y-4">
              ...
              <div className="space-y-2"><Label>Parent Category</Label>
                <Select defaultValue={editItem.parent.toLowerCase()}><SelectTrigger className="bg-secondary border-border"><SelectValue /></SelectTrigger>
                  <SelectContent>{["—", "Electronics", "Clothing", "Sports"].map(c => <SelectItem key={c} value={c.toLowerCase()}>{c}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <Button className="w-full" onClick={() => setEditItem(null)}>Save Changes</Button>
            </div>
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!deleteItem} onOpenChange={() => setDeleteItem(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">Are you sure you want to delete <strong>{deleteItem?.name}</strong>? This action cannot be undone.</p>
