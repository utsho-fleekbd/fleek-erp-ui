...
const customers = Array.from({ length: 30 }, (_, i) => {
  ...
  return {
    ...
    image: `https://api.dicebear.com/7.x/avataaars/svg?seed=${names[i % names.length]}`,
...
export default function CustomersPage() {
  ...
  const [deleteItem, setDeleteItem] = useState<(typeof customers)[0] | null>(
    null,
  );
                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <img
                        src={c.image}
                        className="flex h-8 w-8 items-center justify-center rounded-full bg-secondary text-xs font-medium"
                      />
        ...
        <DialogContent>
          ...
          {viewItem && (
            <div className="space-y-3">
              ...
              <p className="text-sm">Address: {viewItem.address}</p>
              <p className="text-sm">Joined: {viewItem.joinDate}</p>
              <p className="text-sm">
                Orders: {viewItem.totalOrders} | Returned:{" "}
                {viewItem.returnedOrders}
              </p>
              <p className="text-sm font-mono font-medium">
                Total Paid: ${viewItem.totalPaid.toLocaleString()}
              </p>
            </div>
          )}
        </DialogContent>
