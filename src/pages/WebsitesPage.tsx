...
const templates = [
  {
    ...
    category: "Fashion",
    desc: "Modern fashion e-commerce with lookbook style",
    icon: ShoppingBag,
  },
  {
    id: 2,
    ...
    desc: "Clean electronics store with comparison features",
    icon: Laptop,
  },
  {
    id: 3,
    name: "Casa Modern",
    category: "Furniture",
    desc: "Elegant furniture store with room visualization",
    icon: Armchair,
  },
  {
    id: 4,
    name: "Glow Beauty",
    category: "Cosmetics",
    desc: "Minimalist beauty brand with product showcases",
    icon: Pipette,
  },
];
    ...
    <AppLayout>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          Manage your e-commerce websites
        </p>
      </div>
              ...
              <Card key={w.id} className="border-border bg-card">
                <CardContent className="p-5">
                  <div className="flex items-start justify-between">
                    ...
                    <span
                      className={`inline-flex items-center rounded border px-2 py-0.5 text-[11px] font-medium ${siteStatusStyles[w.status]}`}
                    >
                      {w.status}
                    </span>
                  </div>
          ...
          <Card className="border-border bg-card">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-sm">
                <Sparkles className="h-4 w-4 text-primary" /> AI Website
                Generator
              </CardTitle>
            </CardHeader>
              ...
              {generated && (
                <div className="space-y-4 pt-4">
                  ...
                  <div className="space-y-2">
                    {[
                      "Hero Banner — Full-width image with CTA",
                      "Featured Collections — 3-column grid",
                      "New Arrivals — Product carousel",
                      "Lookbook Section — Masonry gallery",
                      "Newsletter Signup — Email capture",
                      "Footer — Links, social, contact",
                    ].map((section, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-3 rounded border border-border bg-secondary p-3"
                      >
                        <div className="flex h-8 w-8 items-center justify-center rounded bg-primary/10 font-mono text-xs text-primary">
                          {i + 1}
                        </div>
